...
void user_nfc_test(void) {
    int     ret        = 0;
    uint8_t data[22]   = {0xE1, 0x40, 0x40, 0x01, 0x03, 0x0F, 
                          0xD1, 0x06, 0x06, 0x57, 0x4C, 0x43, 0x43, 0x41,
                          0x50, 0x01, 0x57, 0x07, 0x1E, 0x03, 0x0A,
                          0x11};
    uint8_t buffer[64] = {0};

    // user_nfc_init();

    // clear user memory (64 bytes)
    ret = _nfc_write_user_mem(0x0000, buffer, sizeof(buffer));
